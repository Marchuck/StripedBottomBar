 ...
 */
public class StripedBottomBar extends LinearLayout implements View.OnClickListener, View.OnLongClickListener {
    ...
    private StripedBottomBarTab[] currentTabs;

    public StripedBottomBar(Context context) {
        this(context, null);
    }

    public StripedBottomBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public StripedBottomBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs, defStyleAttr, 0);
    }

    @RequiresApi(Build.VERSION_CODES.LOLLIPOP)
    public StripedBottomBar(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void populateAttributes(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        TypedArray ta = context.getTheme()
                               .obtainStyledAttributes(attrs, R.styleable.StripedBottomBar, defStyleAttr, defStyleRes);

        try {
            tabXmlResource = ta.getResourceId(R.styleable.StripedBottomBar_bb_tabXmlResource, 0);
            isTabletMode = ta.getBoolean(R.styleable.StripedBottomBar_bb_tabletMode, false);
            behaviors = ta.getInteger(R.styleable.StripedBottomBar_bb_behavior, BEHAVIOR_NONE);
            inActiveTabAlpha = ta.getFloat(R.styleable.StripedBottomBar_bb_inActiveTabAlpha,
                    isShiftingMode() ? DEFAULT_INACTIVE_SHIFTING_TAB_ALPHA : 1);
            activeTabAlpha = ta.getFloat(R.styleable.StripedBottomBar_bb_activeTabAlpha, 1);
            ...
            longPressHintsEnabled = ta.getBoolean(R.styleable.StripedBottomBar_bb_longPressHintsEnabled, true);
            inActiveTabColor = ta.getColor(R.styleable.StripedBottomBar_bb_inActiveTabColor, defaultInActiveColor);
            activeTabColor = ta.getColor(R.styleable.StripedBottomBar_bb_activeTabColor, defaultActiveColor);
            badgeBackgroundColor = ta.getColor(R.styleable.StripedBottomBar_bb_badgeBackgroundColor, Color.RED);
            hideBadgeWhenActive = ta.getBoolean(R.styleable.StripedBottomBar_bb_badgesHideWhenActive, true);
            titleTextAppearance = ta.getResourceId(R.styleable.StripedBottomBar_bb_titleTextAppearance, 0);
            titleTypeFace = getTypeFaceFromAsset(ta.getString(R.styleable.StripedBottomBar_bb_titleTypeFace));
            showShadow = ta.getBoolean(R.styleable.StripedBottomBar_bb_showShadow, true);
        } finally {
            ta.recycle();
        }
    }
     ...
     * for each tab.
     */
    public void setItems(@XmlRes int xmlRes, StripedBottomBarTab.Config defaultTabConfig) {
        if (xmlRes == 0) {
    ...
    private StripedBottomBarTab.Config getTabConfig() {
        return new StripedBottomBarTab.Config.Builder()
    ...
    private void updateItems(final List<StripedBottomBarTab> bottomBarItems) {
        tabContainer.removeAllViews();

        int index = 0;
        int biggestWidth = 0;

        StripedBottomBarTab[] viewsToAdd = new StripedBottomBarTab[bottomBarItems.size()];

        for (StripedBottomBarTab stripedBottomBarTab : bottomBarItems) {
            StripedBottomBarTab.Type type;

            if (isShiftingMode()) {
                type = StripedBottomBarTab.Type.SHIFTING;
            } else if (isTabletMode) {
                type = StripedBottomBarTab.Type.TABLET;
            } else {
                type = StripedBottomBarTab.Type.FIXED;
            }

            if (isIconsOnlyMode()) {
                stripedBottomBarTab.setIsTitleless(true);
            }

            stripedBottomBarTab.setType(type);
            stripedBottomBarTab.prepareLayout();

            if (index == currentTabPosition) {
                stripedBottomBarTab.select(false);

                handleBackgroundColorChange(stripedBottomBarTab, false);
            } else {
                stripedBottomBarTab.deselect(false);
            }

            if (!isTabletMode) {
                if (stripedBottomBarTab.getWidth() > biggestWidth) {
                    biggestWidth = stripedBottomBarTab.getWidth();
                }

                viewsToAdd[index] = stripedBottomBarTab;
            } else {
                tabContainer.addView(stripedBottomBarTab);
            }

            stripedBottomBarTab.setOnClickListener(this);
            stripedBottomBarTab.setOnLongClickListener(this);
            index++;
        }
    ...
    private void resizeTabsToCorrectSizes(StripedBottomBarTab[] tabsToAdd) {
        ...
        for (StripedBottomBarTab tabView : tabsToAdd) {
    ...
    /**
     * Set a listener that gets fired when the selected {@link StripedBottomBarTab} is about to change.
     *
    ...
    /**
     * Set a listener that gets fired when the selected {@link StripedBottomBarTab} changes.
     * <p>
    ...
    /**
     * Set a listener that gets fired when a currently selected {@link StripedBottomBarTab} is clicked.
     *
    ...
    public void selectTabAtPosition(int position, boolean animate) {
        ...
        StripedBottomBarTab oldTab = getCurrentTab();
        StripedBottomBarTab newTab = getTabAtPosition(position);
     ...
     */
    public StripedBottomBarTab getCurrentTab() {
     ...
     */
    public StripedBottomBarTab getTabAtPosition(int position) {
        ...
        return (StripedBottomBarTab) child;
    }
     ...
     */
    public StripedBottomBarTab getTabWithId(@IdRes int tabId) {
        return (StripedBottomBarTab) tabContainer.findViewById(tabId);
    }
    ...
    public void setInActiveTabAlpha(float alpha) {
        ...
        batchPropertyApplier.applyToAllTabs(new BatchTabPropertyApplier.TabPropertyUpdater() {
            @Override
            public void update(StripedBottomBarTab tab) {
    ...
    private void updateTitleBottomPadding() {
        ...
        for (int i = 0; i < tabCount; i++) {
            StripedBottomBarTab tab = getTabAtPosition(i);
    ...
    public void onClick(View target) {
        if (!(target instanceof StripedBottomBarTab)) return;
        handleClick((StripedBottomBarTab) target);
    }

    @Override
    public boolean onLongClick(View target) {
        return !(target instanceof StripedBottomBarTab) || handleLongClick((StripedBottomBarTab) target);
    }

    private StripedBottomBarTab findTabInLayout(ViewGroup child) {
        for (int i = 0; i < child.getChildCount(); i++) {
            View candidate = child.getChildAt(i);

            if (candidate instanceof StripedBottomBarTab) {
                return (StripedBottomBarTab) candidate;
            }
        }
    ...
    private void handleClick(StripedBottomBarTab newTab) {
        StripedBottomBarTab oldTab = getCurrentTab();
    ...
    private boolean handleLongClick(StripedBottomBarTab longClickedTab) {
    ...
    private void shiftingMagic(StripedBottomBarTab oldTab, StripedBottomBarTab newTab, boolean animate) {
        if (isShiftingMode()) {
    ...
    private void handleBackgroundColorChange(StripedBottomBarTab tab, boolean animate) {
